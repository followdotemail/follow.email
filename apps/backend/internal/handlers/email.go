...
// GetEmailByID handles requests to get a specific email by ID
func (h *EmailHandler) GetEmailByID(c *gin.Context) {
	...
	// Query email by ID and user ID - only select required columns for performance
	var email models.Email
	if err := h.db.Select("id, user_id, s3_body_key, s3_attachments_key").Where("id = ? AND user_id = ?", emailID, userID).First(&email).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
...
// GetEmailContent handles requests to get email content from S3
func (h *EmailHandler) GetEmailContent(c *gin.Context) {
	// Get user ID from context (set by auth middleware)
	clerkID, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "User not authenticated"})
	...
	// Query email by ID and user ID - get full email record
	var email models.Email
	...
	// Debug logging
	log.Printf("[DEBUG] GetEmailContent - Email ID: %s, MessageID: %s, UserID: %s", email.ID, email.MessageID, userID)
	log.Printf("[DEBUG] gmailSyncService is nil: %v", h.gmailSyncService == nil)

	// Check prerequisites
	if email.MessageID == "" {
		log.Printf("[ERROR] MessageID is empty for email %s", email.ID)
	...
	if h.gmailSyncService == nil {
		log.Printf("[ERROR] Gmail sync service not initialized")
	...
	log.Printf("[DEBUG] Fetching email body from Gmail for MessageID: %s", email.MessageID)
	body, err := h.gmailSyncService.GetEmailContentFromGmail(ctx, userID, email.MessageID)
	if err != nil {
		log.Printf("[ERROR] Failed to fetch email body from Gmail: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("Failed to fetch email content from Gmail: %v", err),
		})
		return
	}
	log.Printf("[SUCCESS] Fetched email body from Gmail, length: %d bytes", len(body))

	// Fetch attachments from Gmail API only
	log.Printf("[DEBUG] Fetching attachments from Gmail for MessageID: %s", email.MessageID)
	attachments, err := h.gmailSyncService.GetEmailAttachmentsFromGmail(ctx, userID, email.MessageID)
	if err != nil {
		log.Printf("[ERROR] Failed to fetch attachments from Gmail: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("Failed to fetch attachments from Gmail: %v", err),
		})
		return
	}
	log.Printf("[SUCCESS] Fetched %d attachments from Gmail", len(attachments))

	// Build attachment list
	attachmentList := []gin.H{}
	for _, att := range attachments {
		attachmentList = append(attachmentList, gin.H{
			"filename":      att.Filename,
			"content_type":  att.MimeType,
			"size":          att.Size,
			"attachment_id": att.AttachmentID,
			"download_url":  fmt.Sprintf("/api/v1/emails/%s/attachments/%s", email.ID, att.AttachmentID),
		})
	}

	// Return response with Gmail data only
	c.JSON(http.StatusOK, gin.H{
		"email_id":    email.ID,
		"body":        body,
...
func (h *EmailHandler) DownloadAttachment(c *gin.Context) {
	...
	if email.MessageID != "" && h.gmailSyncService != nil {
		...
		if err != nil {
			log.Printf("Failed to download attachment from Gmail: %v", err)
...
func (h *EmailHandler) SendEmail(c *gin.Context) {
	...
	log.Printf("[DEBUG] Sending email for user %s to %v", userID, req.To)
	...
	log.Printf("[SUCCESS] Email sent successfully. MessageID: %s, ThreadID: %s", messageID, threadID)
